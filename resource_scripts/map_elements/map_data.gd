...
@export var boss_level_grow: int
	...
	match room.room_type:
		room.Type.NORMAL:
			if (room.explored and randi_range(1, 3) == 1) or room.explored == false:
				room.enemy = GameAPI.get_normal_enemy()
		...
		room.Type.BOSS:
			room.enemy.grow_levels(boss_level_grow)
